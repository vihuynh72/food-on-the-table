...
import { MapPin, Phone, ExternalLink, Clock, CheckCircle2, Navigation2, Check } from "lucide-react";

import type { DonationLocation } from "@/components/donation/DonationMap";
import { addDonationEntry, getDonationCountForLocation } from "@/lib/donationLog";
...
export function DonationLocationCard({ location, selected, onSelect }: DonationLocationCardProps) {
  ...
  const donationCount = getDonationCountForLocation(location.id);

  const handleLogDonation = () => {
    const updatedLog = addDonationEntry({ locationId: location.id, locationName: location.name, notes });
    setDialogOpen(false);
    setNotes("");
    toast({
      title: "Donation logged",
      description: `Thanks for donating to ${location.name}! (${updatedLog.length} total entries tracked)`,
    });
  };
  ...
  return (
    ...
    >
      ...
      <CardContent className="space-y-4">
        ...
        {donationCount > 0 && (
          <div className="flex items-center gap-2 text-xs text-woodland">
            <Check className="w-4 h-4" />
            You have logged {donationCount} donation{donationCount > 1 ? "s" : ""} here.
          </div>
        )}
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent onClick={(e) => e.stopPropagation()}>
            <DialogHeader>
              ...
              <DialogDescription>
                Confirm your donation at {location.name}. Add an optional note about what you donated.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-2">
              <Input
                placeholder="What did you donate? (optional)"
                value={notes}
            ...
            <DialogFooter>
              ...
              <Button onClick={handleLogDonation} className="bg-asparagus hover:bg-woodland">
                Confirm
              </Button>
            </DialogFooter>
          </DialogContent>
