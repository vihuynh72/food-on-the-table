...
export function DonationMap({
  ...
  const apiKey =
    ...
    (__NEXT_PUBLIC_GOOGLE_MAPS_API_KEY__ as string | undefined);
  ...
  useEffect(() => {
    if (!apiKey) {
      setLoadError(
        "Google Maps API key not configured. Set VITE_GOOGLE_MAPS_API_KEY (or NEXT_PUBLIC_GOOGLE_MAPS_API_KEY).",
      );
      return;
    }
